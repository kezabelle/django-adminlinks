...
class ObjectDeleteAdmin(RequiresContextAuthMixin, InclusionTag):
    name = 'admin_delete'
    ...
    def get_template(self, context, **kwargs):
        ...
        opts = kwargs['object']._meta
        if not context['request'].user.has_perm(opts.app_label + '.' + opts.get_delete_permission()):
            return 'adminlinks/bad_context.html'

        if kwargs['template'] is not None:
            return kwargs['template']

        return super(ObjectDeleteAdmin, self).get_template(context, **kwargs)
...
class ObjectHistoryAdmin(RequiresContextAuthMixin, InclusionTag):
    ...
    def get_template(self, context, **kwargs):
        ...
        return super(ObjectHistoryAdmin, self).get_template(context, **kwargs)
